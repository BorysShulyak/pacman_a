...
import random
from Astar import a_star, manhattan
...
class MapCreator:
    ...
    def create_maze_kruskal(self, rows, cols):
        ...
        self.add_coins(COIN_AMOUNTS)
        if a_star(self.grid, START, DESTINATION, manhattan) is not None:
            return self.grid
        else:
            self.grid = self.create_maze_kruskal(rows, cols)
            return self.grid

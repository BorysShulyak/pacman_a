...
class Player:
    def __init__(self, application, pos):
        self.algo_to_use = ONLY_WAY
        self.heuristic = greedy
        ...
        if self.algo_to_use == All_COINS:
            self.path = self.grab_all()
        elif self.algo_to_use == ONLY_WAY:
            self.path = a_star(self.application.grid_map, START, DESTINATION, self.heuristic)
    ...
    def update(self):
        ...
        if new_pos != self.grid_pos:
            ...
            if self.algo_to_use == All_COINS:
                ...
                self.path = a_star(self.application.grid_map, (int(self.grid_pos[1]), int(self.grid_pos[0])), DESTINATION, self.heuristic)
    ...
    def draw_path(self):
        for point in self.path:
            pygame.draw.rect(self.application.screen, (0, 200, 200),
                               (point[1] * self.application.cell_width + BORDER // 2 + 4,
                                point[0] * self.application.cell_height + BORDER // 2 + 4,
                                self.application.cell_width - 9,
                                self.application.cell_height - 9))
    ...
    def grab_all(self):
        ...
        ways = []
        for coin in coins:
            way = a_star(self.application.grid_map, hero, coin, self.heuristic)
            if way is not None:
                ways += way
            else:
                print("RELOAD GAME")
                return [START]
        return ways + a_star(self.application.grid_map, hero, self.destination, self.heuristic)
